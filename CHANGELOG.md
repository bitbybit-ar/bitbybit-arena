...
- **`verifyHashtagPost` now matches mixed-case hashtag posts.** Participants on the same challenge publish from clients that don't all normalize `t` tags — we observed the same hashtag arriving as `pizzaday`, `PizzaDay`, and `PizzaDayXLaCrypta` from different posters, but most relays treat NIP-01 `#t` filter values as case-sensitive bytes, so only the lowercase / uppercase / capitalized variants the verifier explicitly enumerated were ever found. The verifier now runs a two-stage strategy: a fast targeted `#t` query first (cheap path for the canonical lowercase posters), then a broader fallback that drops the `#t` filter, streams the author's recent kind:1 notes, and post-filters case-insensitively. Both stages share a client-side predicate so a relay returning a non-matching kind:1 doesn't poison the result. `fetchFirstMatchingEvent` gained an optional `predicate` arg that keeps the helper listening when an event fails the post-filter (instead of resolving on the first relay event regardless of fitness).
