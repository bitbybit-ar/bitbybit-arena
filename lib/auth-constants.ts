 ...
 * (`proxy.ts`) can import without pulling in `next/headers`, which
 * the edge runtime rejects. `lib/auth.ts` re-exports these so server
 * callers can keep the single import path.
 */
